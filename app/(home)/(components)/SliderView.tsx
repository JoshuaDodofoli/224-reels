...
export default function SliderView() {
  ...
  // Per-slide refs: outer = clip-path wrapper, inner = scale wrapper
  ...
  useGSAP(() => {
    ...
    // ── Initial states: ALL slides start fully hidden ────────────────
    ...
    // SplitText all title overlays, start hidden
    const allTexts = textRefs.current.filter(Boolean);
    const split = new SplitText(allTexts, { type: 'chars', charsClass: 'char' });
    gsap.set(split.chars, { yPercent: 100 });

    // ── Intro reveal: slide 0 enters like the about-page GIF ─────────
    ...
    // Outer clip-path opens (bottom → top reveal)
    intro.to(firstOuter, { clipPath: 'inset(0% 0% 0% 0%)', duration: 1.1 });
    // Inner scale settles simultaneously — depth illusion
    ...
    const handleScroll = (direction: number) => {
      ...
      const prevChars = textRefs.current[prevIdx]?.querySelectorAll('.char');
      const nextChars = textRefs.current[nextIdx]?.querySelectorAll('.char');

      // Direction-aware clip values
      const enterFrom = direction > 0 ? 'inset(100% 0% 0% 0%)' : 'inset(0% 0% 100% 0%)';
      const exitTo    = direction > 0 ? 'inset(0% 0% 100% 0%)' : 'inset(100% 0% 0% 0%)';

      // Prepare incoming slide on top
      gsap.set(nextOuter, { clipPath: enterFrom, zIndex: 2 });
      gsap.set(nextInner, { scale: 1.1 });
      if (nextChars?.length) {
        gsap.set(nextChars, { yPercent: direction > 0 ? 100 : -100 });
      }

      const tl = gsap.timeline({
        onComplete: () => {
          // Reset the now-hidden old slide for future use
          gsap.set(prevOuter, { clipPath: 'inset(100% 0% 0% 0%)', zIndex: 0 });
          gsap.set(nextOuter, { zIndex: 1 });
          isAnimating.current = false;
        },
      });

      // Exit — old slide text wipes out
      if (prevChars?.length) {
        tl.to(prevChars, {
      ...
      // Exit — old slide clips away / Enter — new slide reveals simultaneously
      tl.to(prevOuter, {
        clipPath: exitTo,
        duration: 0.85,
        ease: 'power4.inOut',
      ...
      // Inner scale settles as mask opens — depth illusion
      tl.to(nextInner, {
        scale: 1,
        duration: 0.85,
        ease: 'power4.inOut',
  ...
  useEffect(() => {
    ...
    tl.to([counterRef.current, dateRef.current], {
      y: exitY, duration: 0.2, ease: 'power2.in', stagger: 0.025,
    });
      ...
      <div className="w-full md:w-1/2">
        ...
        <div className="relative aspect-video overflow-hidden">
          {reels.map((reel, idx) => (
            ...
            >
              {/* Outer — clip-path mask, all start hidden (intro animates slide 0 in) */}
              <div
                ref={(el) => { outerRefs.current[idx] = el; }}
                className="absolute inset-0"
                style={{ clipPath: 'inset(100% 0% 0% 0%)' }}
              >
                {/* Inner — scale wrapper (depth illusion) */}
                <div
